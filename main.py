...
def certificate(cor: int, inc: int, all_q: int) -> None:
    ...
    print(
        f"| Percentage of correct answers: {colored(f'{perc}', 'light_blue')}%{abs(len(str(perc)) - 6) * ' '}|"
    )
...
try:
    if __name__ == "__main__":
        ...
        print(
            colored(
                "If you can use two words in the same form, use: '/'", "light_magenta"
            )
        )
        while tnss:
            main()
except KeyboardInterrupt:
    print("GoodBye. Here your result:\n")
finally:
